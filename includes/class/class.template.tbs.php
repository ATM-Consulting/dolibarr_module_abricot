...
class TTemplateTBS {
	...
	public static function convertToPDF($file,$extension='',$TParam=array()) {
		...
		if(getDolGlobalString('ABRICOT_CONVERTPDF_USE_ONLINE_SERVICE')) {
			...
			//print USE_ONLINE_SERVICE;
			$result = file_get_contents(getDolGlobalString('ABRICOT_CONVERTPDF_USE_ONLINE_SERVICE'), false, $context);
			//exit($result);
			...
			if($extension == '.html' || $extension == '.php') {
				...
				$wkhtmltopdf->_bin = getDolGlobalString('ABRICOT_WKHTMLTOPDF_CMD') ? getDolGlobalString('ABRICOT_WKHTMLTOPDF_CMD') : 'wkhtmltopdf';
				...
				$cmd_print = !getDolGlobalString('ABRICOT_CONVERTPDF_CMD') ? 'libreoffice --invisible --norestore --headless --convert-to pdf --outdir' : getDolGlobalString('ABRICOT_CONVERTPDF_CMD');
