...
class TPDOdb
{
    ...
    function __construct($db_type = '', $connexionString = '', $DB_USER = '', $DB_PASS = '', $DB_OPTIONS = array())
    {
        ...
        if (empty($connexionString)) {
            ...
            if (!empty($port)) {
                $this->connexionString .= ';port=' . $port;
            }
            if (!empty($charset) && empty($conf->global->ABRICOT_USE_OLD_DATABASE_ENCODING_SETTING)) {
                $this->connexionString .= ';charset=' . $charset;
            }

            if (defined('DB_SOCKET') && constant('DB_SOCKET') != '') {
                $this->connexionString .= ';unix_socket=' . DB_SOCKET;
            }
    ...
    function showTrace()
    {
        ...
        foreach ($trace as $row) {
            if ((!empty($row['class']) && $row['class'] == 'TPDOdb')
                ...
                || (!empty($row['function']) && $row['function'] == 'call_user_func')) {
                continue;
            }
    ...
    function Execute($sql, $TBind = array())
    {
        ...
        if (!empty($this->db->errorCode)) {
            if ($this->debug) {
                $this->Error("PDO DB ErrorExecute : " . print_r($this->db->errorInfo(), true) . ' ' . $this->query);
            }
            //return(mysql_errno());
        }
    ...
    function dbupdate($table, $value, $key)
    {
        ...
        if ($this->debug) {
            $this->Error("Mise à jour (" . (int) $res . " ligne(s)) " . $this->query);
        }
    ...
    function dbinsert($table, $value)
    {
        ...
        if ($this->debug) {
            $this->Error("Insertion " . $this->query);
        }
    ...
    function Get_All($mode = PDO::FETCH_OBJ, $functionOrClassOrColumn = null)
    {
        ...
        if (!is_null($functionOrClassOrColumn)) {
            return $this->rs->fetchAll($mode, $functionOrClassOrColumn);
        } else {
            if ($this->rs === false) {
                return array();
            } else {
                return $this->rs->fetchAll($mode);
            }
        }
    ...
    function Get_field($pField)
    {
        ...
        if (isset($this->currentLine->{$pField})) {
            return $this->currentLine->{$pField};
        } else {
            return false;
        }
    ...
    function Get_column_list($table, $alias = '')
    {
        ...
        foreach ($TColumns as $col) {
            if (!empty($alias)) {
                $TFields[] = $alias . '.' . $col->{$colfield} . ' AS "' . $alias . '.' . $col->{$colfield} . '"';
            } else {
                $TFields[] = $col->{$colfield};
            }
        }
